...
exports.addProjects= async(req,res)=>{
    console.log("Inside Add Projects API");
    ...
    try {
        ...
        if (existingProject) {
                     res.status(406).json("Project already exists in our collection !! plaease uplaod another!!!")
...
exports.editProject=async(req,res)=>{
      ...
      const uplaodImage=req.file?req.file.filename:projectImage
      const userId=req.payload
      const {pid} = req.params
       try {
        const updateProject=await projects.findByIdAndUpdate({_id:pid},{
          title,overview,languages,github,website,projectImage:uplaodImage,userId
        },{new:true})
